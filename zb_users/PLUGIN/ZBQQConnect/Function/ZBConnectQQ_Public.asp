...
'****************************************************************************
Public Sub logout()
	'Session(ZC_BLOG_CLSID&"ZBQQConnect_strAccessToken")=""
	'Session(ZC_BLOG_CLSID&"ZBQQConnect_strOpenID")=""
	''数据库代码在这里加入
...
Function API(url,json,httptype)
	...
	elseif strHttptype="POST&" then
		API=fakeQQConnect.PostHttp(Murl)
	end if
	AddDebug "Result="&API
	tempid=""
	aryMutliContent=""
...
Function Authorize()
	...
	Authorize=a&"?"&c'fakeQQConnect.GetHttp(a&"?"&c)
End Function
'*******************************************************************************
'** CallBack                                                         **
'*******************************************************************************
...
Function GetOpenId(AccessToken)
	...
	OpenId=Split(Split(fakeQQConnect.GetHttp(a&b),"""openid"":""")(1),"""")(0)
