...
<%For x=0 To i-1%>
	...
	for (a = 0; a < l; a++) {
		...
		d<%=x%>.push('<td class="cke_dark_background cke_centered" style="vertical-align: middle;" role="presentation"><a href="javascript:void(0)" role="option"', ' aria-posinset="' + (a + 1) + '"', ' aria-setsize="' + l + '"', ' aria-labelledby="' + m + '"', ' class="cke_smile cke_hand" tabindex="-1" onkeydown="CKEDITOR.tools.callFunction( ', n, ', event, this );">', '<img class="cke_hand" title="', h[a].substr(0,h[a].length-4), '" cke_src="', p<%=x%> + h[a], '" alt="', h[a].substr(0,h[a].length-4), '"', ' src="', p<%=x%> + h[a], '"', CKEDITOR.env.ie ? " onload=\"this.setAttribute('width', 2); this.removeAttribute('width');\" ": "", '><span id="' + m + '" class="cke_voice_label">' + e.smiley_descriptions[a] + "</span></a>", "</td>");
		a % g == g - 1 && d<%=x%>.push("</tr>")
	}
	if (a < g - 1) {
