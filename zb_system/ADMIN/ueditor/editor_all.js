...
//                img.style.cssText += ';margin:' + (first.margin||0) +"px;" + 'border:' + (first.border||0) +"px solid #000";
            ...
            if(opt.length == 1){
                str = '<img src="'+ci.src+'" '+ (ci.data_ue_src ? ' data_ue_src="' + ci.data_ue_src +'" ':'') +
                        ...
                        (ci.title?' title="'+ci.title+'"':' title="Image"') + 
						(ci.title?' alt="'+ci.title+'"':' alt="Image"')+
						' border="'+ (ci.border||0) + '" hspace = "'+(ci.hspace||0)+'" vspace = "'+(ci.vspace||0)+'" />';
            ...
            str = str.replace( /<!--\s*EndFragment\s*-->[\s\S]*$/, '' )
                ...
                .replace(/<v:shape [^>]*>[\s\S]*?.<\/v:shape>/gi,function(str){
                    try{
                        ...
                        return '<img width="'+ptToPx(width)+'" height="'+ptToPx(height)+'" src="' + src + '" alt="image"  title="image"/>'
                    } catch(e){
                        return '';
                    }
...
(function (){
    ...
    var btnCmds = ['undo', 'redo','formatmatch',
        ...
        'deletecol','splittorows','splittocols','splittocells','mergecells','deletetable','insertintro'];
...
UE.commands['insertintro']={execCommand:function(cmdName){var me=this;if(me.queryCommandState(cmdName)!==-1){if(me.document.getElementsByClassName("more").length!=0){me.execCommand("insertHtml","<hr>")}else{me.execCommand("insertHtml","<hr class=\"more\"/>")}var range=me.selection.getRange(),start=range.startContainer;if(start.nodeType==1&&!start.childNodes[range.startOffset]){var tmp;if(tmp=start.childNodes[range.startOffset-1]){if(tmp.nodeType==1&&tmp.tagName=="HR"){if(me.options.enterTag=="p"){tmp=me.document.createElement("p");range.insertNode(tmp);range.setStart(tmp,0).setCursor()}else{tmp=me.document.createElement("br");range.insertNode(tmp);range.setStartBefore(tmp).setCursor()}}}}return true}    }};