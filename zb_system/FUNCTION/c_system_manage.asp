...
'∟
	...
	Response.Write "<table border=""1"" width=""100%"" cellspacing=""1"" cellpadding=""1"">"
	Response.Write "<tr><td width=""18""></td><td>"& ZC_MSG076 &"</td><td>"& ZC_MSG079 &"</td><td>"& ZC_MSG001 &"</td><td>"& ZC_MSG147 &"</td><td></td><td></td></tr>"
			...
			If CheckPluginState(objXmlFile.documentElement.selectSingleNode("id").text) Then
				If BlogUser.Level<=CInt(objXmlFile.documentElement.selectSingleNode("level").text) Then
					If fso.FileExists(BlogPath & "zb_users/plugin/" & f1.name & "/" & objXmlFile.documentElement.selectSingleNode("path").text) Then
						s=s & "<a href=""../../ZB_USERS/plugin/" & f1.name & "/" & objXmlFile.documentElement.selectSingleNode("path").text &""">[" & ZC_MSG022 & "]</a>"
					End If
				End If
			Else
			End If
...
<%
	...
	t3=t1-t2:t2=0:t1=0
	  ...
	  for xhf=0 to ubound(sptzsx)
	  	if sptzsx(xhf)=".." then t2=t2+1 else t1=t1+1
	  next
	  if t1-t2<t3 Then Response.Write  "<p>当前路径:" & path & "</p><p>对不起，为了您的其他程序的安全，您只能修改Z-Blog文件夹内的文件，同时也不允许修改Global.asa和Global.asax。</p><p><a href='../cmd.asp?act=SiteFileMng&path="&Server.URLEncode(BlogPath)&"'>点击这里返回</a></p></div>" :Response.end
...
Function ExportSiteFileEdit(tpath,opath)
	...
	if instr(chkg,"global.asa") Then
		Response.Write  "<p>当前文件:" & chkg & "</p><p>对不起，为了您的其他程序的安全，您只能修改Z-Blog文件夹内的文件，同时也不允许修改Global.asa和Global.asax。</p><p><a href='../cmd.asp?act=SiteFileMng&path="&Server.URLEncode(oPath)&"'>点击这里返回</a></p></div>" :Response.end
	End If
	Call GetBlogHint()
