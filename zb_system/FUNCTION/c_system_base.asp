...
'	逻辑处理
	...
	objRS.ActiveConnection=objConn
	objRS.Source=""
	objRS.Open("SELECT [log_ID],[log_Tag],[log_CateID],[log_Title],[log_Intro],[log_Content],[log_Level],[log_AuthorID],[log_PostTime],[log_CommNums],[log_ViewNums],[log_TrackBackNums],[log_Url],[log_Istop],[log_Template],[log_FullUrl],[log_Type],[log_Meta] from [blog_Article] where ([log_Type]=0) And ([log_Level]>2) And ([log_PostTime] BETWEEN "& ZC_SQL_POUND_KEY &y&"-"&m&"-1"& ZC_SQL_POUND_KEY &" AND "& ZC_SQL_POUND_KEY &ny&"-"&nm&"-1"& ZC_SQL_POUND_KEY &") AND ([log_PostTime]<=" & ZC_SQL_POUND_KEY & FormatDateTime(Now) & ZC_SQL_POUND_KEY & ")")
...
Function RefreshOptionFormFileToDB()
	...
	For Each a In BlogConfig.Meta.Names
		...
		End If
	Next
	Call BlogConfig.Write("ZC_BLOG_VERSION","2.0 Beta2 Build 121113")
