...
Const ZC_MSG017="显示"
Const ZC_MSG018="来宾"
Const ZC_MSG019="超级管理%s登录%s验证身份%s注销%s后台管理%s发表评论%s查看权限%s查看RSS输出%s批量操作%s索引重建%s文章重建%s管理文章%s编辑文章%s发布文章%s删除文章%s管理分类%s修改分类%s删除分类%s管理评论%s删除评论%s管理用户%s修改用户%s创建用户%s删除用户%s附件管理%s附件上传%s附件删除%s站内搜索%sTag管理%sTag编辑%sTag保存%sTag删除%s网站设置%s网站设置保存%s插件管理%s侧栏模块管理%s"
...
Const ZC_MSG216="在正文插入摘要分隔线'─'可以让系统生成摘要内容.如果需指定摘要内容,请点击 "
...
Const ZC_MSG222=" 提示:需要先建立一个分类才能发表文章."
...
Const ZC_MSG256="无内容"
Const ZC_MSG257="启用静态日志隐匿访问功能(开启匿名功能时自定义静态日志目录配置里必须包含有{%id%}或{%alias%})"
Const ZC_MSG258="自动保存成功"
Const ZC_MSG259="自定义静态日志目录配置(可以是{%post%},{%category%},{%user%},{%year%},{%month%},{%day%},{%id%},{%alias%}之间的组合,可以用/分隔,系统初始化配置是{%post%})"
Const ZC_MSG260="登录"
Const ZC_MSG261="修改时间"
Const ZC_MSG262="关闭评论功能"
Const ZC_MSG263="启用自定义静态日志目录功能"
Const ZC_MSG264="取消"
...
Const ZC_MSG270="所属文章ID:(一般情况下请勿改动此数字)"
Const ZC_MSG271="启用评论倒序输出"
Const ZC_MSG272="评论编辑"
Const ZC_MSG273=" 之前的批量操作还未全部执行,请点击""提交""按钮执行完所有的操作,或是点是""取消""按钮清除批量操作队列."
