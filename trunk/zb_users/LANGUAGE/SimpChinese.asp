...
Const ZC_MSG018="来宾"
Const ZC_MSG019="登录%s验证身份%s注销%s后台管理%s发表评论%s传送引用%s查看权限%s索引重建%s文章重建%s管理文章%s编辑文章%s发布文章%s删除文章%s管理分类%s修改分类%s删除分类%s管理评论%s删除评论%s回复评论%s管理引用%s删除引用%s发送引用%s管理用户%s修改用户%s创建用户%s删除用户%s附件管理%s附件上传%s附件删除%s站内搜索%sTag管理%sTag编辑%sTag保存%sTag删除%s网站设置%s网站设置保存%s插件管理%s查看RSS和ATOM输出%s站内文件管理%s站内文件编辑%s站内文件保存%s站内文件删除%s超级管理%s"
Const ZC_MSG020="注销"
