...
function ubb2html(sUBB)
{
	...
	sHtml=sHtml.replace(/\[font-color\s*=\s*([^\]"]+?)(?:"[^\]]*?)?\s*\]/ig,'<font color="$1">');
	sHtml=sHtml.replace(/\[font-face\s*=\s*([^\]"]+?)(?:"[^\]]*?)?\s*\]/ig,'<font face="$1">');
	sHtml=sHtml.replace(/\[\/(color|font)\]/ig,'</font>');
	sHtml=sHtml.replace(/\[font-size\s*=\s*([^\]"]+?)(?:"[^\]]*?)?\s*\]/ig,function(all,size){
		if(size.match(/^\d+$/))size=arrFontsize[size-1];
		return '<span style="font-size:'+size+';">';
	});
	sHtml=sHtml.replace(/\[font-back\s*=\s*([^\]"]+?)(?:"[^\]]*?)?\s*\]/ig,'<span style="background-color:$1;">');
	...
	sHtml=sHtml.replace(/\[f\]\s*([^\]"]+?)(?:"[^\]]*?)?\s*\/\[\/f\\]/ig,'<img emot="$1" />');
...
function html2ubb(sHtml)
{
	...
	function font2ubb(all,tag,attrs,content)
	{
		...
		if(match){
			arrStart.push('[font-face='+match[1]+']');
			arrEnd.push('[/font-face]');
		}
		match=attrs.match(/ size\s*=\s*"\s*(\d+)\s*"/i);
		if(match){
			arrStart.push('[font-size='+match[1]+']');
			arrEnd.push('[/font-size]');
		}
		match=attrs.match(/ color\s*=\s*"\s*([^"]+)\s*"/i);
		if(match){
			arrStart.push('[font-color='+formatColor(match[1])+']');
			arrEnd.push('[/font-color]');
		}
	...
	for(i=0;i<3;i++)sUBB=sUBB.replace(/<(span)(?:\s+[^>]*?)?\s+style\s*=\s*"((?:[^"]*?;)*\s*(?:font-family|font-size|color|background|background-color)\s*:[^"]*)"(?: [^>]+)?>(((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S]|<\1(\s+[^>]*?)?>((?!<\1(\s+[^>]*?)?>)[\s\S])*?<\/\1>)*?<\/\1>)*?)<\/\1>/ig,function(all,tag,style,content){
		...
		var arrStart=[],arrEnd=[];
		if(face){
			arrStart.push('[font-face='+face[1]+']');
			arrEnd.push('[/font-face]');
		}
		if(size){
			arrStart.push('[font-size='+size[1]+']');
			arrEnd.push('[/font-size]');
		}
		if(color){
			arrStart.push('[font-color='+formatColor(color[1])+']');
			arrEnd.push('[/font-color]');
		}
		if(back){
			arrStart.push('[font-back='+formatColor(back[1])+']');
			arrEnd.push('[/font-back]');
		}
