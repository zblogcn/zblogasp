...
Class YT_Alipay
	Private sPartner
	Private sKey
	Private sSeller_Email
	Private sInput_Charset
	Private sNotify_Url
	Private sReturn_Url
	Private sPost_Url
	Private sResult_Url
	...
	Public Property Get Seller_Email
		Seller_Email = sSeller_Email
	End Property
	Public Property Get TransAction_Type
		TransAction_Type = sTransAction_Type
	End Property
	Public Property Get Status
		Status = sStatus
	End Property
	Public Property Get Input_Charset
		Input_Charset = sInput_Charset
	End Property
	...
	Private Sub Class_Initialize()
		...
		sTransAction_Type = Array("CREATE_PARTNER_TRADE_BY_BUYER","CREATE_DIRECT_PAY_BY_USER")
				...
				If Not Node Is Nothing Then
					...
					Set Object = jsonToObject(Json)
						If GetFieldValue(Object,"YT_Money") Then
							If isEmpty(GetFieldValue(Object,"YT_Logistics_Type")) Then
								GetAlipayType=1
							Else
								GetAlipayType=0
							End If
	...
	Function Create_direct_pay_by_user()
		...
		Call CheckParameter(id,"int",0)
			...
			Set Node = YTModelXML.GetModel(objArticle.CateID)
				...
				Set Object = jsonToObject(Json)
					...
					AddPara "total_fee",GetFieldValue(Object,"YT_Money")
				Set Object = Nothing
			Set Node = Nothing
			body = FilterSQL(Request.Form("YT_Body"))
			paymethod = FilterSQL(Request.Form("YT_Paymethod"))
			AddPara "show_url",ZC_BLOG_HOST&"view.asp?id="&id
		...
		Dim Sql,FieldName,FieldValue,j(),k,x,jsonBody
			...
			jsonBody="["&Join(j,",")&"]"
			FieldName = Array("[OrderID]","[OrderName]","[Body]","[Status]","[Time]","[log_ID]")
			FieldValue = Array(Chr(39)&GetDateTime&Chr(39),Chr(39)&objArticle.Title&Chr(39),Chr(39)&jsonBody&Chr(39),0,Chr(39)&Now()&Chr(39),id)
	...
	Function ArrayPreserve(Byref a,Byval t,Byval v)
		...
		j=UBound(a)+1
		Redim Preserve a(j)
		a(j)="{Text:"&Chr(34)&jsEscape(t)&Chr(34)&",Value:"&Chr(34)&jsEscape(v)&Chr(34)&"}"
	End Function
	'担保交易函数
		...
		Dim id,Json,Object,Node
			...
			Set Node = YTModelXML.GetModel(objArticle.CateID)
				...
				Set Object = jsonToObject(Json)
					...
					AddPara "price",GetFieldValue(Object,"YT_Money")
					AddPara "logistics_fee",GetFieldValue(Object,"YT_Logistics_Fee")
			...
			body = FilterSQL(Request.Form("YT_Body"))
			quantity = Request.Form("YT_Quantity")
			...
			AddPara "body",body									'备注
			AddPara "out_trade_no",GetDateTime
			AddPara "quantity",quantity							'商品数量
			...
			If (Not objRS.bof) And (Not objRS.eof) Then
				...
				For i = 1 To objRS.PageSize
					...
					For Each Field In objRS.Fields
						...
						Else
							R(j) = Field.Name&":"&Chr(34)&jsEscape(objRS(Field.Name))&Chr(34)
						End If
					Next
	...
	Function GetFieldValue(Byref Object,FieldName)
		...
		For Each Field In Object
			If UCase(Field.Name) = UCase(FieldName) Then
	...
	function get_http()
		...
		gateway = YT_ALIPAY_RESULT_URL &"partner=" & Partner & "&notify_id=" & request("notify_id")
