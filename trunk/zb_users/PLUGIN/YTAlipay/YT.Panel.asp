...
YT.Panel.Alipay={
	YT_ALIPAY_TRANSACTION_TYPE:['担保交易','即时交易']	,
	...
	M:function(n){
				...
				try{
					...
					var t = $('#aListView').find('table').clone();
						...
						for(var i=0;i<d.objRow.length;i++){
							var YT_ALIPAY_TYPE = GetAlipayType(eval('('+unescape(d.objRow[i].Body)+')'));
							var r = t.find('tr').eq(1).clone();
								...
								r.find('td').eq(4).find('a:last').click(function(){
									var YT_ALIPAY_TYPE = GetAlipayType(eval('('+unescape(d.objRow[$(this).attr('rel')].Body)+')'));
									...
									var jsonBody = unescape(d.objRow[$(this).attr('rel')].Body);
										...
										for(var j=0;j<_d.length;j++){
											...
											if(unescape(_d[j].Text) == '交易类型'){
												_r.find('td:last').html(YT.Panel.Alipay.YT_ALIPAY_TRANSACTION_TYPE[_d[j].Value]);
											}else{
																...
																$.ajax({
																	...
																	success: function(s) {
																		...
																		if(s[0]=='WAIT_BUYER_CONFIRM_GOODS'){
																			_t.find('tr').eq(2).find('td').eq(1).text(YT.Panel.Alipay.TRADE_STATUS[YT_ALIPAY_TYPE][2].Text).attr('title',YT.Panel.Alipay.TRADE_STATUS[YT_ALIPAY_TYPE][2].Intro);
		...
		function GetAlipayType(d){
			for(var j=0;j<d.length;j++){
				if(unescape(d[j].Text) == '交易类型'){
					return d[j].Value;	
				}
			}
		}
