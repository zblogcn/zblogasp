...
<% End If %>
                      ...
                      <div id="divContent" style="clear:both;">
						<p style="text-align:left;"><span class='editinputname'><%=ZC_MSG055%>:</span>&nbsp;&nbsp;<span id="timemsg"></span><span id="msg2"></span><span id="msg"></span><span class='editinputname'></span>
						<script type="text/javascript" src="c_autosaverjs.asp?act=edit&amp;type=ueditor"></script></p>
                        
                        <textarea id="editor_ke" name="txaContent" style="display:none"><%=TransferHTML(EditArticle.Content,"[textarea]")%></textarea><div id="contentready"><img id="statloading" src="../../../zb_system/image/admin/loading.gif"/><%=ZC_MSG276%></div>
                      ...
                      <div id="divIntro" style="display:<%If EditArticle.Intro="" Then Response.Write "none" Else Response.Write "block"%>;">
                        ...
                        <textarea id="editor_ke2" name="txaIntro" style="display:none"><%=TransferHTML(EditArticle.Intro,"[textarea]")%></textarea><div id="introready"><img id="statloading" src="../../../zb_system/image/admin/loading.gif"/><%=ZC_MSG276%></div>
                      </div>
...
function checkArticleInfo(){
	document.getElementById("edit").action="../../../zb_system/cmd.asp?act=ArticlePst&webedit=ueditor<%=IIF(Request.QueryString("type")="Page","&type=Page","")%>";
...
$('#showtags').click(function (event) {  
	...
	if(loaded==false){$.getScript('../../../zb_system/function/c_admin_js.asp?act=tags');loaded=true;}
});  
...
function AutoIntro() {
	...
	if(s.indexOf("<hr class=\"more\" />")>-1){
		editor2.html(editor.html().split("<hr class=\"more\" />")[0]);
	}else{
		s="";
		var ss=editor.html().split("</p>");
		for (var t in ss){
