...
<br/>catalog.html模板处请用&lt;#articlelist/author/meta/abc#&gt;这样的标签用来显示自定义数据内容.
<%
Set m=New TMeta
m.LoadString=c.Read("UserMeta")
If UBound(m.Names)>0 Then
	...
	For i=LBound(m.Names)+1 To UBound(m.Names)
	Response.Write "<input style='width:258px' type='text' readonly='readonly' value='&lt;#article/author/meta/"&m.Names(i)&"#&gt;' /> "
	Response.Write "<input style='width:278px' type='text' readonly='readonly' value='&lt;#articlelist/author/meta/"&m.Names(i)&"#&gt;' /> <span >("&m.GetValue(m.Names(i))&")</span><br/>"
	Next
	Response.Write "</p>"
End If
%>
